...
async function abrirUnidad(unidad) {
  ...
  return (
    <div style={{ background: '#f0f4f8', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#022847', height: '56px', padding: '0 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <img src="/logo.png" alt="CostaMarket" style={{ height: '34px', width: 'auto' }} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.08)', border: '0.5px solid rgba(255,255,255,0.15)', borderRadius: '20px', padding: '4px 10px 4px 5px' }}>
            <div style={{ width: '22px', height: '22px', borderRadius: '50%', background: '#0D6CB0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: '500', color: 'white' }}>{iniciales}</div>
            <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.8)' }}>{perfil?.nombre?.split(' ')[0]}</span>
          </div>
          <button onClick={logout} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.4)', fontSize: '11px', padding: '4px' }}>Salir</button>
        </div>
      </div>

      <div style={{ maxWidth: '980px', margin: '0 auto', padding: '2rem 1rem 1.25rem' }}>
        <div style={{ fontSize: '10px', fontWeight: '500', letterSpacing: '0.1em', color: '#0D6CB0', textTransform: 'uppercase', marginBottom: '6px' }}>Panel de control · CostaMarket</div>
        <h1 style={{ fontSize: '22px', fontWeight: '500', color: '#022847', margin: '0 0 4px' }}>
          Bienvenido{perfil?.nombre ? `, ${perfil.nombre.split(' ')[0]}.` : '.'}
        </h1>
      </div>

      <div style={{ maxWidth: '980px', margin: '0 auto', padding: '0 1rem 1rem', display: 'grid', gridTemplateColumns: cols, gap: '12px' }}>
        {unidades.map(unidad => {
          ...
          return (
            <div
              ...
              style={{ background: 'white', border: '0.5px solid #d4e0eb', borderRadius: '14px', overflow: 'hidden', cursor: bloqueada ? 'default' : 'pointer', opacity: bloqueada ? 0.6 : 1, transition: 'border-color 0.15s' }}
